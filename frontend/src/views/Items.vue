    ...
    <section class="catalog">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        >
          ...
          <div class="mt-3">
            <button
              class="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 disabled:opacity-60"
              :disabled="!it.available"
              @click="abrirModal(it.id)"
            >
              Solicitar préstamo
            </button>
          </div>
      ...
      <div class="relative bg-white w-full max-w-md mx-4 rounded-xl shadow-xl p-6">
        ...
        <form @submit.prevent="confirmarPrestamo" class="space-y-4">
          ...
          <div class="flex items-center justify-end gap-3">
            <button type="button" class="px-4 py-2 rounded border" @click="cerrarModal" :disabled="submitting">
              Cancelar
            </button>
            <button
              type="submit"
...
async function confirmarPrestamo() {
  ...
  try {
    await httpClient.post('/api/v1/loan', {
      userId,
...
<style scoped>
.catalog { max-width: 1200px; margin: 0 auto; padding: 1rem; }
...
.row { display: grid; grid-template-columns: 100px 1fr; gap: .5rem; margin: .25rem 0; }
